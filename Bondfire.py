...
def cnn_model_fn(features, labels, mode):
    ...
    input_layer = tf.reshape(features, [-1, 54, 54, 1])
...
#     train_labels = np.random.randint(1, high=5, size=len(star_ratings))
    ...
    classifier = learn.Estimator(model_fn=cnn_model_fn, model_dir="/tmp/convnet_model")
    ...
    eval_results = classifier.evaluate(x=train_data[-100:], y=train_labels[-100:], metrics=metrics)
